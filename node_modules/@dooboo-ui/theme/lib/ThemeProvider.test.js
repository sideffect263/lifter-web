...
const integerBetween = (min, max) => Math.floor(Math.random() * (max - min + 1)) + min;
describe('doobooContext', () => {
    context('when theme provider exists', () => {
        describe('themeType, theme, colors, changeThemeType', () => {
            ...
            const wrapper = ({ children }) => (_jsx(ThemeProvider, { initialThemeType: themeType, customTheme: { dark: customDarkTheme }, children: children }));
        ...
        describe('media', () => {
            const wrapper = ({ children }) => (_jsx(ResponsiveContext.Provider, { value: given.deviceInfo, children: _jsx(ThemeProvider, { children: children }) }));
            let media;
            beforeEach(() => {
                const { result } = renderHook(() => useTheme(), { wrapper });
