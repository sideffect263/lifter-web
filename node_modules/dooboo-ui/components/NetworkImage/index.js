...
function NetworkImage(props) {
    ...
    const defaultImageStyle = useMemo(() => css({
        aspectRatio: 110 / 74,
        width: '50%',
        height: 'auto',
        maxHeight: '50%',
    ...
    const renderFallback = useCallback(() => {
        return isValidElement(props?.loadingSource) ? (props?.loadingSource) : (<Image style={[defaultImageStyle]} source={props.loadingSource ?? logo} resizeMethod="resize" resizeMode="cover"/>);
    }, [defaultImageStyle, logo, props.loadingSource]);
    const renderImage = useCallback(() => (<Image key={url} style={[{ flex: 1, alignSelf: 'stretch' }]} source={{ uri: url }} resizeMethod="resize" resizeMode="cover" {...imageProps}/>), [imageProps, url]);
    useEffect(() => {
        if (!url) {
            return;
        }
