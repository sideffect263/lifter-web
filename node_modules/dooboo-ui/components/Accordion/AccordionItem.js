...
const AccordionItem = (props) => {
    ...
    const { testID, data: item, shouldAnimate = true, collapseOnStart = true, animDuration = 200, activeOpacity = 1, toggleElement = <StyledIcon name="chevron-down-light" theme={theme}/>, renderTitle = (title) => <StyledTitle theme={theme}>{title}</StyledTitle>, renderBody = (body) => <StyledItem theme={theme}>{body}</StyledItem>, dropDownAnimValue, styles, style, } = props;
    const { titleContainer = { backgroundColor: theme.role.primary }, bodyContainer = { backgroundColor: theme.bg.basic }, } = styles ?? {};
    ...
    const renderIndicator = (element) => (<Animated.View style={{
            position: 'absolute',
            right: 20,
            transform: [
                {
    ...
    return (<Animated.View style={[
      ...
      <TitleContainer theme={theme} testID={`title_${testID}`} onPress={handlePress} activeOpacity={activeOpacity} style={titleContainer}>
        <>
      ...
      <Animated.View testID={`body_${testID}`} style={{
            height: bodyMounted
                ...
                : undefined,
        }} onLayout={handleBodyLayout} accessibilityState={{
            expanded: !collapsed,
        }}>
        {item.bodies.map((body, key) => (<ItemContainer key={key} style={bodyContainer}>
            {renderBody(body)}
          </ItemContainer>))}
      </Animated.View>
    </Animated.View>);
