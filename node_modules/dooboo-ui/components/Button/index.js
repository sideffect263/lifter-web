...
const ButtonStyles = ({ theme, type = 'solid', color = 'primary', size = 'medium', loading, disabled, }) => {
...
export const Button = (props) => {
    const { testID, type = 'solid', color = 'primary', size = 'medium', disabled, loading = false, loadingElement, text, startElement, endElement, style, styles, onPress, activeOpacity = 0.6, touchableOpacityProps, borderRadius, } = props;
    const ref = useRef(null);
    const hovered = useHover(ref);
    const { theme } = useTheme();
    const { padding, backgroundColor, borderColor, borderWidth, textColor, disabledBackgroundColor, disabledBorderColor, disabledTextColor, } = ButtonStyles({ theme, type, color, size, loading, disabled });
    ...
    const renderContainer = (children) => {
        return (<ButtonContainer testID={loading ? 'loading-view' : 'button-container'} style={[
    ...
    const renderChild = () => (<>
      {startElement}
      <Text style={[compositeStyles.text, disabled && compositeStyles.disabledText]}>
        {text}
      </Text>
      {endElement}
    </>);
