...
const ProgressCircleComponent = ({ style, styles, progress, type = 'info', }) => {
    ...
    return (<Container onLayout={(e) => {
            const { height, width } = e.nativeEvent.layout;
            setContainerLayout({ width, height });
        }} style={[containerLayout, style, styles?.container]}>
      <Svg style={{ position: 'absolute' }} width={containerLayout.width} height={containerLayout.height} viewBox={`0 0 ${containerLayout.width} ${containerLayout.height}`}>
        <AnimCircle 
    // @ts-ignore => PR given https://github.com/react-native-svg/react-native-svg/pull/1822
    fill="transparent" r={radius} strokeWidth={strokeWidth} stroke={strokeColor} rotation={-90} strokeDasharray={2 * Math.PI * radius} strokeDashoffset={animValue.current.interpolate({
            inputRange: [0, 1],
            outputRange: [2 * Math.PI * radius, 0],
        })} {...circleProps}/>
        <SvgCircle fill="transparent" r={radius + strokeWidth / 2} strokeWidth={1} stroke={strokeColor} {...circleProps}/>
      </Svg>
